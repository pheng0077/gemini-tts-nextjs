...
import { GEMINI_VOICES, MODELS, AudioItem, VoiceOption } from '@/lib/types';
...
async function generateSpeechAPI({ text, voice, model, systemInstruction }: { 
  text: string; 
  voice: string; 
  model: string; 
  ...
  const response = await fetch('/api/generate', {
    ...
    body: JSON.stringify({ text, voice, model, systemInstruction }),
  });
...
async function fetchVoicesAPI(): Promise<VoiceOption[]> {
...
export default function Home() {
  ...
  const [selectedVoice, setSelectedVoice] = useState(GEMINI_VOICES[2].id); // Default to Charon
  ...
  useEffect(() => {
    const processQueue = async () => {
        ...
        try {
            const wavBlob = await generateSpeechAPI({
                text: nextItem.text,
  ...
  const handleDownloadAll = async () => {
      ...
      try {
          ...
          await Promise.all(successItems.map(async (item) => {
              if (!item.audioUrl || !folder) return;
              try {
    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto">
      <header className="mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-zinc-800 pb-4">
        ...
        <div className="flex gap-2">
            <span className="text-xs bg-zinc-900 border border-zinc-800 text-zinc-500 px-3 py-1 rounded-full">v2.0.0</span>
        </div>
      </header>
              ...
              <div className="flex items-center justify-between mb-4 bg-zinc-950 pb-2 z-10 shrink-0">
                 ...
                 {pendingCount > 0 && (
                    <button 
                        ...
                        className={`text-xs px-3 py-1.5 rounded-full flex items-center gap-1 font-medium transition-all ${
                            isProcessing 
