...
import { PlayIcon, PauseIcon, TrashIcon, DownloadIcon, SparklesIcon, BoltIcon, ArrowPathIcon } from '@/components/Icons';
...
export const HistoryItem: React.FC<HistoryItemProps> = ({ item, onDelete, onGenerate }) => {
  ...
  const formatModelName = (modelId: string) => {
    return modelId
      ...
      .replace('-tts', '')
      .replace(/-/g, ' ')
      .replace(/\w/g, c => c.toUpperCase()); // Capitalize first letters
  };
  ...
  return (
    <div className={`border rounded-xl p-4 transition relative overflow-hidden ${
        isGenerating ? 'bg-zinc-900/50 border-indigo-500/50' : 
